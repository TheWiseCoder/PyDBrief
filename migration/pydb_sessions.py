...
def delete_session(session_id: str,
    ...
    if curr_state in [SessionState.ACTIVE, SessionState.INACTIVE,
                      SessionState.ABORTED, SessionState.FINISHED]:
...
def get_active_session(client_id: str) -> str | None:
    ...
    for key, value in migration_registry.items():
        if value.get(MigSpec.STATE) == SessionState.ACTIVE and \
...
def set_session_state(input_params: dict[str, Any],
                      errors: list[str]) -> SessionState | None:

    # initialize the return variable
    result: SessionState | None = None
    ...
    if isinstance(is_active, bool):
        new_state = SessionState.ACTIVE if is_active else SessionState.INACTIVE
        ...
        if not new_session:
            ...
            if not session_registry or not \
                ...
                errors.append(validate_format_error(141,
                                                    ...
                                                    f"@{MigSpec.SESSION_ID}"))
        if not errors:
            state: SessionState = get_session_state(session_id=session_id)
            if (state == SessionState.ABORTING and not get_session) or \
                    (state == SessionState.MIGRATING and not (get_session or del_session)):
...
def abort_session_migration(session_id: str,
    ...
    session_registry: dict[StrEnum, Any] = migration_registry.get(session_id)
    if session_registry.get(MigSpec.STATE) == SessionState.MIGRATING:
        session_registry[MigSpec.STATE] = SessionState.ABORTING
        result = True
    else:
        # 142: Invalid value {}: {}
        errors.append(validate_format_error(142,
...
def assert_session_abort(session_id: str,
    ...
    if session_registry.get(MigSpec.STATE) == SessionState.ABORTING:
