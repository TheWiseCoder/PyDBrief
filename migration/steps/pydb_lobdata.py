            ...
            if is_lob(column_type):
                ...
                for item in (session_specs[MigSpec.NAMED_LOBDATA] or []):
                    ...
                    if item.startswith(f"{table_name}.{column_name}="):
                        return_column = item[item.index("=")+1:]
                        break
                lob_columns.append((column_name, return_column))
        ...
        if lob_columns:
            ...
            if not target_s3:
                if db_table_exists(errors=errors,
                    ...
                    continue
                if not pk_columns:
                    warn_msg: str = (f"Table {source_db}.{source_table} is not "
                                     "eligible for LOB migration to another database (no PKs)")
                    migration_warnings.append(warn_msg)
                    logger.warning(msg=warn_msg)
                    # skip table migration
                    continue
            ...
            for lob_column, return_column in lob_columns:
                ...
                if target_s3:
                    if not return_column and not pk_columns:
                    ...
                    if not pk_columns:
                        warn_msg: str = ("Expecting an index to exist on column "
                                         f"{source_db}.{source_table}.{return_column}, as table has no PKs")
                        migration_warnings.append(warn_msg)
                        logger.warning(msg=warn_msg)
                        pk_columns = [return_column]
